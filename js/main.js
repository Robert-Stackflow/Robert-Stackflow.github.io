    ...
    window.refreshFn = function() {
        ...
        setInterval(function() {
            ...
            if (rmf.day_night_count + initDay == 0)
                GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow("冬至·伊始:我的心与爱是不是能够这般纯粹，经受住时空的考验"), rmf.day_night_count++;
            if (rmf.day_night_count + initDay == 20)
                GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow("交替·新生:月徊而霜凝兮，良人伴我侧；月斜而影绰兮，明镜照我心"), rmf.day_night_count++;
            if (rmf.day_night_count + initDay == 60)
                GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow("风起·渴盼:桐絮扰动着尘世，恋人缔造着世界；细腻不语的青苔，是我对你的爱恋"), rmf.day_night_count++;
            if (rmf.day_night_count + initDay == 120)
                GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow("雾霭·探索:越来越浓的雾霭模糊着彼此的视线，越来越厚的障壁阻隔着彼此的心儿"), rmf.day_night_count++;
            if (rmf.day_night_count + initDay == 200)
                GLOBAL_CONFIG.Snackbar !== undefined && btf.snackbarShow("拨云·成长:生活不全都是恋爱，恋爱却全都是生活"), rmf.day_night_count++;
        ...
        if (document.getElementById("site-title") && (document.getElementById("site-title").innerHTML.trim() == document.querySelector("#menus > div.menus_items > div:nth-child(5) > a > span").innerHTML.trim())) {
            ...
            document.querySelectorAll(".el-textarea__inner").forEach(item => { item.setAttribute("placeholder", "输入您的留言") });
        }
