AV.init({
  appId: "fffJo68x4OzaTPt2xt9Ir0Ak-gzGzoHsz",
  appKey: "AXzwa52w3PfcFQ9m6DAURKJq",
  serverURL: "https://fffjo68x.lc-cn-n1-shared.com",
});
function lunbo() {
  var speaks = [];
  ...
  query.find().then((talks) => {
    ...
    for (i = talks.length - 1; i >= talks.length - 31; i--) {
      ...
      var bbcontent = talks[i]["attributes"]["content"]
        .replace(/<[^>]+>/g, "")
        .replace(/<[^>]+>/g, "")
        .replace(
          /(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+\.(png|jpg|jpeg|webp)/g,
          "[图片]"
        )
        .replace(
          /(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+/g,
      ...
      if (days > 31) {
        speaks.push(
          String(talks[i]["createdAt"].Format("yyyy-MM-dd")) + "：" + bbcontent
        );
      } else if (days > 0) {
